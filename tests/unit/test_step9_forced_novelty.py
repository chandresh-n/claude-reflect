...
(src/claude_reflect/agents/proposer.py) must expose a function that
...
class TestForcedNoveltyRollLogic:
    ...
    def test_roll_below_threshold_triggers_forced_novelty(self):
        ...
        from claude_reflect.agents.proposer import check_forced_novelty

        # Mock random() to return 0.1 — below 0.2 threshold
        with patch("claude_reflect.agents.proposer.random.random", return_value=0.1):
    ...
    def test_roll_above_threshold_does_not_trigger(self):
        ...
        with patch("claude_reflect.agents.proposer.random.random", return_value=0.5):
            result = check_forced_novelty(
                forced_novelty_probability=0.2,
                null_baseline_probability=0.01,
            )
    ...
    def test_roll_at_exact_threshold_does_not_trigger(self):
        ...
        with patch("claude_reflect.agents.proposer.random.random", return_value=0.2):
            result = check_forced_novelty(
                forced_novelty_probability=0.2,
                null_baseline_probability=0.01,
            )
    ...
    def test_null_baseline_triggered_when_second_roll_below_threshold(self):
        ...
        with patch(
            "claude_reflect.agents.proposer.random.random",
    ...
    def test_zero_probability_never_triggers(self):
        ...
        with patch("claude_reflect.agents.proposer.random.random", return_value=0.0):
            result = check_forced_novelty(
